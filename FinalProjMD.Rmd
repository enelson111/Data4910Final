...
wdi_vars <- names(wdi_data_default) %>% setdiff(c("Country", "Year")) #extracts names of all columns and removes country+year for indicator variables

wdi_years <- sort(unique(wdi_data_default$Year)) #extracts and sorts year
  ...
  tabPanel( #starts defining wdi tab
    ...
    sidebarLayout( #starts the sidebar and main panel layout
      sidebarPanel( #starts the left side panel for user controls
        ...
        wellPanel( #start panel to group the custom file upload controls
            h4("Custom File"), #title
            fileInput("wdi_file", "Upload Custom WDI CSV (Optional)", accept = ".csv"), #file input for CSV
            conditionalPanel( #panel appears if condition is met
                condition = "output.wdi_custom_uploaded", #condition is file must be uploaded
                hr(),
                h5("Map Your Variables"),#heading
                uiOutput("wdi_country_map"), #placeholder to map country
                uiOutput("wdi_year_map"), #placeholder to map year
                uiOutput("wdi_indicator_maps"),
                actionButton("wdi_apply", "Apply Configuration", class = "btn-primary"), #button to trigger mapping/file upload
                hr()
            )
        )),
...
server <- function(input, output, session) { #defines the server function
  wdi_data <- reactiveVal(wdi_data_default) #stores currently active dataset
  wdi_raw_data <- reactiveVal(NULL) #holds the raw uploaded user data
  wdi_custom_uploaded <- reactiveVal(FALSE) #determines if data is the default or user uploaded
  ...
  observe({ #reacts to changes in wdi_data() and updates ui controls based on the new data dimensions
    df <- wdi_data() #retrieve current wdi data frame from the reactive value
    countries <- sort(unique(df$Country)) #extract and sort unique country names
    years <- sort(unique(df$Year)) #extract and sort unique Year values
    indicators <- setdiff(names(df), c("Country", "Year")) #find all columns that are indicators (not country/year)
  
    updateSelectizeInput(session, "wdi_countries", choices = countries, selected = head(countries, 3)) #update country selection input with new list
    updateSliderInput(session, "wdi_years", min = min(years), max = max(years), value = c(min(years), max(years))) #update year slider range and default value
    updateSelectInput(session, "wdi_indicator", choices = indicators, selected = indicators[1]) #update single indicator select input
    updateSelectizeInput(session, "wdi_table_cols", choices = indicators, selected = head(indicators, 4)) #update table column selection input
  
    updateNumericInput(session, "wdi_map_yr", #update map year input
      value = max(years), #set default map year to the latest year
      min = min(years), #set minimum year for the map input
      max = max(years)) #set maximum year for the map input
  })
  
  output$wdi_custom_uploaded <- reactive({ !is.null(wdi_raw_data()) }) #reactive true if a custom file has been uploaded 
  outputOptions(output, "wdi_custom_uploaded", suspendWhenHidden = FALSE) #ensure the output is updated
  
  observeEvent(input$wdi_file, { #triggered when a file is uploaded
    req(input$wdi_file) #require a file object exists
  
    df <- read.csv(input$wdi_file$datapath, stringsAsFactors = FALSE) #read uploaded CSV file into a data frame
    wdi_raw_data(df) #store raw uploaded data frame in reactive value
    wdi_custom_uploaded(TRUE) #set true to display the mapping controls
    showNotification("File uploaded successfully. Map your variables now.", type = "message") #notify user of successful upload
  })
  output$wdi_country_map <- renderUI({ #render ui for selecting the country column from the uploaded file
    req(wdi_raw_data()) #require raw data has been uploaded
    cols <- names(wdi_raw_data()) #get column names of raw data
    selectInput("wdi_country_col", "Country Column:", choices = cols, selected = cols[1]) #dropdown to map country column
  })
  output$wdi_year_map <- renderUI({ #renders ui for selecting the year column from the uploaded file
    req(wdi_raw_data()) #require raw data has been uploaded
    cols <- names(wdi_raw_data()) #get column names of the raw data
    selectInput("wdi_year_col", "Year Column:", choices = cols, selected = cols[2]) #dropdown to map year column
  })
  output$wdi_indicator_maps <- renderUI({ #render ui for selecting indicator columns (gdp, life exp., pop.)
    req(wdi_raw_data()) #require raw data has been uploaded
    cols <- names(wdi_raw_data()) #get column names of raw data
    indicator_cols <- setdiff(cols, c(input$wdi_country_col, input$wdi_year_col)) #columns remaining after selecting country and year
  
    tagList( #group multiple ui elements together
      selectInput("wdi_gdp_col", "GDP Column:", choices = c("None", indicator_cols), selected = "None"), #dropdown to map gdp column
      selectInput("wdi_life_exp_col", "Life Expectancy Column:", choices = c("None", indicator_cols), selected = "None"), #dropdown to map life exp column
      selectInput("wdi_pop_col", "Population Column:", choices = c("None", indicator_cols), selected = "None") #dropdown to map pop column
    )
  })
  
  observeEvent(input$wdi_apply, { #triggered when apply configuration button is clicked
    req(wdi_raw_data(), input$wdi_country_col, input$wdi_year_col) #require raw data and country/year mapping inputs
  
    df <- wdi_raw_data() #get raw uploaded data
    
    mapping_vec <- c( #start building vector for column renaming
      "Country" = input$wdi_country_col, #map country column to user selected column
      "Year" = input$wdi_year_col #map year column to user selected column
    )
    
    if (input$wdi_gdp_col != "None") mapping_vec["GDP_current_US"] <- input$wdi_gdp_col #add gdp if selected
    if (input$wdi_life_exp_col != "None") mapping_vec["life_expectancy_at_birth"] <- input$wdi_life_exp_col #add life exp if selected
    if (input$wdi_pop_col != "None") mapping_vec["population"] <- input$wdi_pop_col #add pop if selected
    
    df <- df %>% rename(!!!mapping_vec) #apply renaming vector to the data frame
    
    if ("Year" %in% names(df)) { #check if year exists
      df <- df %>% #data cleaning pipeline
        mutate(Year = suppressWarnings(as.integer(Year))) %>% #convert year to integer
        filter(!is.na(Year)) #filter out rows where the year conversion failed
    }

    wdi_data(df) #update main reactive value with the newly cleaned and mapped data frame
    
    countries <- sort(unique(df$Country)) #extract and sort unique country names from the new data
    years <- sort(unique(df$Year)) #extract and sort unique year values from the new data
    indicators <- setdiff(names(df), c("Country", "Year")) #determine the set of available indicators
    
    updateSelectizeInput(session, "wdi_countries", choices = countries, selected = head(countries, 3)) #update country dropdown
    updateSliderInput(session, "wdi_years", min = min(years), max = max(years), value = c(min(years), max(years))) #update year slider range and value
    updateSelectInput(session, "wdi_indicator", choices = indicators, selected = indicators[1]) #update the single indicator dropdown
    updateSelectizeInput(session, "wdi_table_cols", choices = indicators, selected = head(indicators, 4)) #update table column dropdown
    
    updateNumericInput(session, "wdi_map_yr", #update map year selection input
      value = max(years), #set default map year to the max year
      min = min(years), #set minimum map year
      max = max(years)) #set maximum map year
    
    showNotification("Custom WDI data configured successfully! App charts and tables are updated.", type = "message") #notify user of successful mapping
  })
    wdi_filtered <- reactive({ #reactive expression to filter the wdi data based on user input
    df <- wdi_reactive() #get the most current wdi data
    req(input$wdi_countries, input$wdi_years) #require country and year selection inputs to proceed
    df %>%
      filter(Country %in% input$wdi_countries, #filter by selected countries
             Year >= input$wdi_years[1], Year <= input$wdi_years[2]) #filter by selected year range
  })
  ...
  output$wdi_map <- renderLeaflet({ #renders the interactive wdi map
    ...
    l <- leaflet(world_join) %>% addTiles() %>% #initializes leaflet with the joined map data and tiles
    ...
    ###define year annotation
    year_label <- paste0( #start defining the html string for the year watermark
      '<div style="', #start html div with inline css
      '  position: fixed; ', #make position fixed relative to the view
      '  top: 50%; ', #center vertically
      '  left: 50%; ', #center horizontally
      '  transform: translate(-50%, -50%); ', #adjust for element size to truly center
      '  margin-left: 12.5%; ', #shift right to visually center in the main panel area
      '  opacity: 0.2; ', #make the text semi-transparent
      '  font-size: 100px; ', #make the font very large
      '  font-weight: bold; ', #make the font bold
      '  color: #333; ', #set the text color
      '  pointer-events: none; ', #prevent text from blocking mouse events on the map
      '  z-index: 999;', #ensure the text is above other map elements
      '">', 
      input$wdi_map_yr, #insert the selected year value
      '</div>' 
    )
    
    #add the custom year watermark control to the map
    l <- l %>% #assign the modified map back to l
      addControl( #add a custom html control element to the leaflet map
        html = year_label, #use the generated year label html
        position = "topleft" 
      )
    l 
  })
